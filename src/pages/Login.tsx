    ...
    <div className="min-h-[100dvh] bg-gradient-to-b from-[#3d004c] via-[#62007a] to-[#f6f7fb] flex justify-center">
      <div className="w-full max-w-[440px] flex flex-col">
        <div className="px-7 pt-[9vh] pb-8 text-white">
          ...
          <p className="text-[13px] text-white/70 mt-1">Sign in to continue to your wallet</p>
        </div>

        <div className="flex-1 rounded-t-[28px] bg-white px-7 pt-8 pb-10 shadow-[0_-8px_30px_rgba(0,0,0,0.15)]">
          <form onSubmit={submit}>
            <label className="block text-[12px] font-semibold text-gray-500 mb-1.5">Phone Number</label>
            <div className="flex items-center bg-[#f7f5fa] border border-transparent focus-within:border-[#62007a] rounded-2xl px-4 py-3.5 mb-5 transition-colors">
            ...
            <div className="flex justify-between items-center mb-7">
              <label className="flex items-center text-[13px] text-gray-600 cursor-pointer">
                <input
                  type="checkbox"
                  checked={savePw}
                  onChange={(e) => setSavePw(e.target.checked)}
                  className="mr-2 w-[16px] h-[16px] accent-[#62007a] cursor-pointer"
                />
                Save Password
              </label>
              <a href="#" className="no-underline text-[#8e24aa] text-[13px] font-medium" onClick={(e) => e.preventDefault()}>
                Forgot password?
              </a>
            </div>
